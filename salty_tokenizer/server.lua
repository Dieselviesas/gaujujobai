...
function secureServerEvent(resource, player, token)
	...
	elseif _source == "" then -- If the request came from the server, then no need to authenticate the token
		...
		if token ~= resourceTokens[resource] then
			if Config.Logs then
				TriggerEvent('hl-logs:tokenizer', _source, 'Aptiktas netinkamas tokenas!\nScriptas: **'..tostring(resource)..'**\nŽaidėjo ID: **'..tostring(_source)..'**\nAtsiųstas tokenas: **'..tostring(token)..'**\nTinkamas tokenas: **'..tostring(resourceTokens[resource])..'**')
			end
			if Config.VerboseServer then
